...
from bimba3d_backend.app.services import project_training_data_update
...
from bimba3d_backend.app.services.project_json import read_json_if_exists as _read_json_if_exists
from bimba3d_backend.app.services.project_json import write_json_atomic as _write_json_atomic
from bimba3d_backend.app.services.project_paths import find_project_dir as _find_project_dir
from bimba3d_backend.app.services.project_shared_config import (
    extract_shared_config_from_params as _extract_shared_config_from_params,
    get_project_shared_config_path as _get_project_shared_config_path,
    merge_shared_config_into_params as _merge_shared_config_into_params,
    read_project_shared_config as _read_project_shared_config,
    write_project_shared_config as _write_project_shared_config,
)
...
from bimba3d_backend.worker.ai_input_modes.legacy_support import normalize_ai_input_mode_token as _normalize_ai_input_mode_token
...
TRAINING_DATA_UPDATE_MARKER_FILE = ".training_data_update_latest.json"
...
CORE_AI_SCORE_PREV_RE = re.compile(r"\bscore_prev=(?P<score>[0-9.eE+-]+|n/a)", re.IGNORECASE)
...
AI_INPUT_MODE_LEARN_RE = re.compile(
    r"AI_INPUT_MODE_LEARN\s+mode=(?P<mode>[^\s]+)\s+preset=(?P<preset>[^\s]+)\s+s_best=(?P<s_best>[0-9.eE+-]+)\s+s_end=(?P<s_end>[0-9.eE+-]+)\s+s_run=(?P<s_run>[0-9.eE+-]+)\s+score=(?P<score>[0-9.eE+-]+)",
    re.IGNORECASE,
)
...
AI_INPUT_MODE_SCORE_OUTCOME_RE = re.compile(
    r"AI_INPUT_MODE_SCORE_OUTCOME\s+mode=(?P<mode>[^\s]+)\s+preset=(?P<preset>[^\s]+)\s+score=(?P<score>[0-9.eE+-]+)\s+score_positive=(?P<score_positive>true|false)",
    re.IGNORECASE,
)
...
def _infer_feature_dependencies(features: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """
    Infer dependency metadata for features showing what each feature depends on.
    """
    ...
    if "focal_length_mm" in features:
        deps["focal_length_mm"] = {
            "type": "exif",
            "sources": ["FocalLength", "CalibratedFocalLength"],
            "defaulted": False,
        }

    # GSD - depends on focal
    if "gsd_median" in features:
        deps["gsd_median"] = {
            "type": "calculated",
            "formula": "(altitude * sensor) / (focal * width)",
            "depends_on": ["altitude", "focal_length", "sensor_width", "image_width"],
            "defaulted": False,
            "reason": None,
        }
...
def _score_reference_step_from_baseline(baseline_comparison: dict[str, Any]) -> int | None:
...
def _loss_at_reference_step(baseline_comparison: dict[str, Any], side: str) -> float | None:
    reference_step = _score_reference_step_from_baseline(baseline_comparison)
    if reference_step is not None:
        value = baseline_comparison.get(f"loss_at_{reference_step}_{side}")
...
def _analytics_metrics(run_analytics: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if summary_metrics:
        ...
        def _summary_final_step() -> int | None:
            for raw in (
                summary.get("tuning", {}).get("end_step") if isinstance(summary.get("tuning"), dict) else None,
                summary.get("major_params", {}).get("total_steps_completed") if isinstance(summary.get("major_params"), dict) else None,
                summary.get("major_params", {}).get("max_steps") if isinstance(summary.get("major_params"), dict) else None,
                canonical.get("final_loss_step"),
            ):
        ...
        elif not isinstance(canonical.get("final_psnr_step"), (int, float)):
            step, _ = _final_from_series(eval_psnr_series)
            canonical["final_psnr_step"] = step if isinstance(step, int) else _summary_final_step()
        ...
        elif not isinstance(canonical.get("final_ssim_step"), (int, float)):
            step, _ = _final_from_series(eval_ssim_series)
            canonical["final_ssim_step"] = step if isinstance(step, int) else _summary_final_step()
        ...
        elif not isinstance(canonical.get("final_lpips_step"), (int, float)):
            step, _ = _final_from_series(eval_lpips_series)
            canonical["final_lpips_step"] = step if isinstance(step, int) else _summary_final_step()
...
def _read_run_config_params(project_dir: Path, run_id: str) -> tuple[dict[str, Any], dict[str, Any]]:
    run_config = _read_json_if_exists(project_dir / "runs" / str(run_id or "") / "run_config.json")
    ...
    return dict(requested), dict(resolved)


def _read_batch_lineage(project_dir: Path) -> dict[str, Any]:
    payload = _read_json_if_exists(project_dir / BATCH_LINEAGE_FILE)
    return payload if isinstance(payload, dict) else {}


def _write_batch_lineage(project_dir: Path, lineage: dict[str, Any]) -> None:
    payload = lineage if isinstance(lineage, dict) else {}
    _write_json_atomic(project_dir / BATCH_LINEAGE_FILE, payload)
...
def _read_project_model_state(project_dir: Path) -> dict[str, Any]:
    payload = _read_json_if_exists(project_dir / PROJECT_MODEL_STATE_FILE)
    if not isinstance(payload, dict):
        return {
            "models": {},
            "active_project_model_key": "",
            "active_project_model_name": "",
        }
    payload.setdefault("models", {})
    payload.setdefault("active_project_model_key", "")
    payload.setdefault("active_project_model_name", "")
    return payload


def _write_project_model_state(project_dir: Path, state: dict[str, Any]) -> None:
    payload = state if isinstance(state, dict) else {}
    payload.setdefault("models", {})
    payload.setdefault("active_project_model_key", "")
    payload.setdefault("active_project_model_name", "")
    payload["updated_at"] = datetime.utcnow().isoformat() + "Z"
    _write_json_atomic(project_dir / PROJECT_MODEL_STATE_FILE, payload)


def _build_project_model_series_key(project_model_name: str) -> str:
    token = _sanitize_run_token(str(project_model_name or "").strip())
    return token or "project_model"
...
def _clear_restart_artifacts(
    ...
    *,
    ...
    if clear_project_level:
        ...
        # Delete project-level EXIF features on full restart (will be re-extracted)
        # This ensures fresh EXIF extraction with the current parser after restart
        if stage == "full":
            targets.append(project_dir / "exif_features.json")
...
def _apply_run_jitter(
    ...
    *,
    ...
    Run index is zero-based; run 1 (index 0) is baseline without jitter for batch runs.
    """
    out = dict(params)
    remarks = []
    ...
    # First run (idx=0) in batch context should be baseline without jitter
    if idx <= 0:
    ...
    if _normalize_jitter_mode(jitter_mode) == "random":
        ...
        if lo > hi:
            lo, hi = hi, lo
        # Apply random jitter for exploration (avoid 1.0)
        while True:
            multiplier = random.uniform(lo, hi)
            if abs(multiplier - 1.0) > 0.05:  # Ensure it's not too close to 1.0
                break
    else:
    ...
    # Track if any parameter goes out of bounds
    out_of_bounds = []
    ...
    for key, (default_val, low, high) in bounded_float_defaults.items():
        base_val = out.get(key, default_val)
        if isinstance(base_val, (int, float)):
            jittered_val = float(base_val) * multiplier
            if jittered_val < low or jittered_val > high:
                out_of_bounds.append(f"{key}={jittered_val:.6f}")
            out[key] = _clamp_float(jittered_val, low, high)

    for key, (default_val, low, high) in bounded_int_defaults.items():
        base_val = out.get(key, default_val)
        if isinstance(base_val, (int, float)):
            jittered_val = round(float(base_val) * multiplier)
            if jittered_val < low or jittered_val > high:
                out_of_bounds.append(f"{key}={jittered_val}")
            out[key] = _clamp_int(jittered_val, low, high)

    # If jitter caused bounds violations, reset to 1.0 and add remark
    if out_of_bounds:
        remarks.append(f"crossed bounds: {', '.join(out_of_bounds[:3])}; jitter reset to 1.0")
        multiplier = 1.0
        # Re-apply with multiplier=1.0
        for key, default_val in lr_defaults.items():
            base_val = params.get(key, default_val)
            if isinstance(base_val, (int, float)):
                out[key] = float(base_val)
        for key, (default_val, low, high) in bounded_float_defaults.items():
            base_val = params.get(key, default_val)
            if isinstance(base_val, (int, float)):
                out[key] = float(base_val)
        for key, (default_val, low, high) in bounded_int_defaults.items():
            base_val = params.get(key, default_val)
            if isinstance(base_val, (int, float)):
                out[key] = int(base_val)

    out["run_jitter_multiplier"] = float(multiplier)
    if remarks:
        existing_remarks = out.get("remarks", "")
        out["remarks"] = ", ".join([existing_remarks] + remarks) if existing_remarks else ", ".join(remarks)

    return out


LEARNING_TABLE_BASELINE_DEFAULTS: dict[str, float] = {
    "feature_lr": 2.5e-3,
    "position_lr_init": 1.6e-4,
    "scaling_lr": 5.0e-3,
    "opacity_lr": 5.0e-2,
...
def _build_learning_param_rows(
    ...
    selected_params_raw: dict[str, Any] | None,
    action_jitter_multiplier: float | None,
    *,
    run_jitter_only: bool = False,
    is_baseline_row: bool = False,
    session_execution_mode: str = "train",
) -> list[dict[str, Any]]:
    jitter = float(action_jitter_multiplier) if isinstance(action_jitter_multiplier, (int, float)) and math.isfinite(action_jitter_multiplier) else 1.0
    ...
    def group_multiplier_for(base_key: str) -> float:
    ...
    if (not actual_params or not isinstance(actual_params, dict)) and is_baseline_row:
        return [
            {
                "key": key,
                "actual": actual,
                "selected_multiplier": 1.0,
                "selected_multiplier_raw": 1.0,
                "jitter": 1.0,
                "log_multiplier": 1.0,
                "final_multiplier": 1.0,
            }
            for key, actual in sorted(LEARNING_TABLE_BASELINE_DEFAULTS.items())
        ]
    ...
    for raw_key, raw_value in actual_params.items():
        ...
        if isinstance(raw_value, (int, float)) and math.isfinite(raw_value):
            if raw_key.endswith("_mult") and isinstance(baseline_value, (int, float)) and math.isfinite(baseline_value):
                # For multiplier keys, convert to actual value
                actual_value: float | None = float(baseline_value) * float(raw_value)
            else:
                ...
                # because actual_params contains pre-jitter values
        ...
        selected_multiplier = float(selected_raw) if isinstance(selected_raw, (int, float)) and math.isfinite(selected_raw) else None
        ...
        selected_multiplier_raw = (
            float(selected_raw_value)
            if isinstance(selected_raw_value, (int, float)) and math.isfinite(selected_raw_value)
            else None
        )
...
def _analytics_ai_insights(run_analytics: dict[str, Any] | None) -> dict[str, Any] | None:
    if not isinstance(run_analytics, dict):
        return None
...
def _extract_event_rows(lines: list[str], row_limit: int) -> list[dict[str, Any]]:
    ...
    for line in lines:
        ...
        if ai_match:
            ...
            score_match = CORE_AI_SCORE_PREV_RE.search(line)
            score_prev = _parse_optional_float(score_match.group("score") if score_match else None)
            ...
            rows.append({
                ...
                "score_from_previous": score_prev,
            })
            continue
        ...
        if features_match:
            ...
            descriptor_count = len([key for key in details.keys() if key != "image_count"])
            rows.append(
                {
                    "timestamp": timestamp,
                    "type": "ai_input_mode_features",
                    "step": None,
                    "summary": f"AI mode {mode}: extracted {descriptor_count} input descriptor{'' if descriptor_count == 1 else 's'}",
                    "mode": mode,
                    "details": details,
                }
            )
        ...
        if learn_match:
            ...
            score = _parse_optional_float(learn_match.group("score"))
            rows.append(
                {
                    "timestamp": timestamp,
                    ...
                    "summary": (
                        ...
                        f"s_run={s_run if s_run is not None else 'n/a'}, score={score if score is not None else 'n/a'}"
                    ),
                    "mode": mode,
        ...
        score_outcome_match = AI_INPUT_MODE_SCORE_OUTCOME_RE.search(line)
        if score_outcome_match:
            mode = str(score_outcome_match.group("mode") or "")
            preset = str(score_outcome_match.group("preset") or "")
            score = _parse_optional_float(score_outcome_match.group("score"))
            score_positive_token = str(score_outcome_match.group("score_positive") or "").strip().lower()
            score_positive = score_positive_token == "true"
            rows.append(
                {
                    "timestamp": timestamp,
                    "type": "ai_input_mode_score_outcome",
                    "step": None,
                    "summary": (
                        f"AI mode score outcome ({mode}): preset {preset}, "
                        f"score={score if score is not None else 'n/a'}, positive={score_positive}"
                    ),
                    "mode": mode,
                    "selected_preset": preset,
                    "score": score,
    ...
    try:
        ...
        for run_dir in sorted([p for p in runs_dir.iterdir() if p.is_dir()], key=lambda p: p.name):
            ...
            if not isinstance(run_analytics, dict):
                continue
            ai_block = run_analytics.get("ai") if isinstance(run_analytics.get("ai"), dict) else {}
            analytics_learning = ai_block.get("input_mode_learning") if isinstance(ai_block.get("input_mode_learning"), dict) else None
            learning_payload = analytics_learning if isinstance(analytics_learning, dict) else {}
            ...
            has_analytics_initial_params = any(learned_input_params.get(k) is not None for k in SELECTOR_OWNED_LEARNED_KEYS)
            if not has_analytics_initial_params:
                learned_input_params = {}
            learned_input_params_source = "run_analytics" if has_analytics_initial_params else "missing_canonical_analytics"
            if isinstance(run_analytics, dict):
                ...
                for cfg in (
                    run_config,
                    run_config.get("resolved_params") if isinstance(run_config, dict) else None,
                    run_config.get("requested_params") if isinstance(run_config, dict) else None,
                ):
                    if isinstance(cfg, dict) and cfg.get("run_jitter_only") is not None:
                        run_jitter_only = bool(cfg.get("run_jitter_only"))
                    if isinstance(cfg, dict) and cfg.get("session_execution_mode") is not None:
                        session_execution_mode = str(cfg.get("session_execution_mode") or "train").strip().lower()
                        break

                ai_input_mode_value = str(insight.get("ai_input_mode") or "").strip().lower()
                has_configured_ai_input_mode = bool(ai_input_mode_value and ai_input_mode_value not in {"not_configured", "none", "null"})
                if has_configured_ai_input_mode or input_mode_learning or learning_payload:
                    ...
                    model_id = (
                        str(insight.get("model_id") or "").strip()
                    ) or None
                    ...
                    rows.append(
                        {
                            ...
                            "score": (input_mode_learning.get("relative_score") if isinstance(input_mode_learning, dict) else None) if has_baseline_comparison else None,
                            ...
                            "run_best_l": (baseline_comparison.get("run_best_l") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "run_best_q": (baseline_comparison.get("run_best_q") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "run_best_t": (baseline_comparison.get("run_best_t") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "run_best_s": (baseline_comparison.get("s_run_best") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "run_best_elapsed": (baseline_comparison.get("run_best_elapsed") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "run_end_l": (baseline_comparison.get("run_end_l") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "run_end_q": (baseline_comparison.get("run_end_q") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "run_end_t": (baseline_comparison.get("run_end_t") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "run_end_s": (baseline_comparison.get("s_run_end") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "run_end_elapsed": (baseline_comparison.get("run_end_elapsed") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "base_best_l": (baseline_comparison.get("base_best_l") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "base_best_q": (baseline_comparison.get("base_best_q") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "base_best_t": (baseline_comparison.get("base_best_t") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "base_best_elapsed": (baseline_comparison.get("base_best_elapsed") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "base_end_l": (baseline_comparison.get("base_end_l") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "base_end_q": (baseline_comparison.get("base_end_q") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "base_end_t": (baseline_comparison.get("base_end_t") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "base_end_elapsed": (baseline_comparison.get("base_end_elapsed") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "time_ref": (baseline_comparison.get("time_ref") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            # Score decomposition (document section 4.X.8).
                            "relative_quality_score": (baseline_comparison.get("r_quality") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "convergence_score": (baseline_comparison.get("r_convergence") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            # AUC is informational only, not used as score.
                            "auc_loss_run": (baseline_comparison.get("auc_loss_run") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "auc_loss_base": (baseline_comparison.get("auc_loss_base") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "score_reference_step": (_score_reference_step_from_baseline(baseline_comparison) if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "loss_at_reference_step_run": (_loss_at_reference_step(baseline_comparison, "run") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            "loss_at_reference_step_base": (_loss_at_reference_step(baseline_comparison, "base") if isinstance(baseline_comparison, dict) else None) if has_baseline_comparison else None,
                            # Offline model info.
                            "exploration_mode": (
                                insight.get("exploration_mode")
                                or (input_mode_learning.get("exploration_mode") if isinstance(input_mode_learning, dict) else None)
                            ),
                            # End offline model fields.
                            "remarks": remarks,
                            ...
                            "learned_input_params_status": (
                                "captured_from_canonical_analytics"
                                if has_analytics_initial_params
                                else "missing_canonical_analytics"
                            ),
                            "final_multiplier_formula": (
                                learning_payload.get("final_multiplier_formula")
                                if isinstance(learning_payload, dict) and isinstance(learning_payload.get("final_multiplier_formula"), str) and learning_payload.get("final_multiplier_formula")
                                else (
                                    "params * selected_multiplier"
                                    if session_execution_mode == "test"
                                    else (
                                        "params * log_multiplier"
                                        if run_jitter_only
                                        else (
                                            "params * selected_multiplier * log_multiplier"
                                            if (
                                                isinstance(learning_payload, dict)
                                                and (
                                                    isinstance(learning_payload.get("selected_multipliers"), dict)
                                                    or isinstance(learning_payload.get("predicted_input_params"), dict)
                                                    or isinstance(learning_payload.get("yhat_scores"), dict)
                                                )
                                            )
                                            else None
                                        )
                            ...
                            "learning_param_rows": _build_learning_param_rows(
                                learned_input_params or None,
                                (
                                    learning_payload.get("selected_multipliers")
                                    if isinstance(learning_payload, dict) and isinstance(learning_payload.get("selected_multipliers"), dict)
                                    else (
                                        learning_payload.get("predicted_input_params")
                                        if isinstance(learning_payload, dict) and isinstance(learning_payload.get("predicted_input_params"), dict)
                                        else (learning_payload.get("yhat_scores") if isinstance(learning_payload, dict) else None)
                                    )
                                ),
                                (
                                    learning_payload.get("selected_multipliers_raw")
                                    if isinstance(learning_payload, dict) and isinstance(learning_payload.get("selected_multipliers_raw"), dict)
                                    else None
                                ),
                                (insight.get("initial_params") or {}).get("run_jitter_multiplier") if isinstance(insight.get("initial_params"), dict) else None,
        ...
        shared_config = _read_json_if_exists(project_dir / "shared_config.json")
        shared_base_run_id = str(shared_config.get("base_run_id") or "").strip() if isinstance(shared_config, dict) else ""
        if shared_base_run_id:
            baseline_ids = [rid for rid in baseline_ids if rid != shared_base_run_id]
            baseline_ids.insert(0, shared_base_run_id)

        existing_row_ids = {str(row.get("run_id") or "").strip() for row in rows}
        for run_dir in sorted([p for p in runs_dir.iterdir() if p.is_dir()], key=lambda p: p.name):
            cfg = _read_json_if_exists(run_dir / "run_config.json")
            requested = cfg.get("requested_params") if isinstance(cfg, dict) and isinstance(cfg.get("requested_params"), dict) else {}
            resolved = cfg.get("resolved_params") if isinstance(cfg, dict) and isinstance(cfg.get("resolved_params"), dict) else {}
            analytics = _read_run_analytics(run_dir)
            summary = analytics.get("summary") if isinstance(analytics, dict) and isinstance(analytics.get("summary"), dict) else {}
            mode_value = str(resolved.get("mode") or requested.get("mode") or summary.get("mode") or "").strip().lower()
            if mode_value == "baseline" and run_dir.name != shared_base_run_id and run_dir.name not in existing_row_ids:
                metrics = _analytics_metrics(analytics)
                run_name = cfg.get("run_name") or cfg.get("name") or requested.get("run_name") if isinstance(cfg, dict) else run_dir.name
                rows.append(
                    {
                        "run_id": run_dir.name,
                        "run_name": run_name or run_dir.name,
                        "ai_input_mode": None,
                        "baseline_run_id": shared_base_run_id or None,
                        "model_id": None,
                        ...
                        "final_lpips": metrics.get("final_lpips"),
                        "score": None,
                        "remarks": "baseline-mode run",
                        "learned_input_params": _extract_learned_params_from_json(analytics, cfg) or None,
                        "learned_input_params_source": "run_config",
                        "learned_input_params_status": "baseline_mode_values",
                        "final_multiplier_formula": "1.0",
                        "learning_param_rows": _build_learning_param_rows(
                            _extract_learned_params_from_json(analytics, cfg) or None,
                            None,
                            None,
                            1.0,
                            run_jitter_only=False,
                ...
                existing_row_ids.add(run_dir.name)
        ...
        if baseline_ids:
            ...
            for baseline_run_id in baseline_ids:
                ...
                # If run_config didn't provide params, try extracting from analytics (for runs without run_config.json)
                if not baseline_learned_input_params:
                    baseline_learned_input_params = _extract_learned_params_from_json(
                        baseline_analytics,
                        baseline_cfg if isinstance(baseline_cfg, dict) else None,
                    )
                ...
                baseline_rows.append(
                    {
                        ...
                        "final_multiplier_formula": "1.0",
                        "learning_param_rows": _build_learning_param_rows(
                            baseline_learned_input_params or None,
                            None,
                            None,
                            1.0,
                            run_jitter_only=False,
        ...
        if shared_base_run_id:
            combined_rows.sort(
                key=lambda row: (
                    0 if row.get("run_id") == shared_base_run_id and row.get("is_baseline_row") else 1,
                    0 if row.get("is_baseline_row") else 1,
        ...
        return {
            ...
            "rows": combined_rows,
            "training_data_update": _read_json_if_exists(project_dir / TRAINING_DATA_UPDATE_MARKER_FILE),
...
def list_projects(include_legacy_colmap_sources: bool = Query(False)):
    ...
    try:
        ...
        def scan_legacy_colmap_source_pipelines(projects_list: list[ProjectListItem]):
            """Temporary scanner for old pipeline folders used only as COLMAP copy sources."""
            # TEMP LEGACY: remove after sparse outputs from old pipelines have been copied
            # into fresh workflow pipelines.
            ...
            env_root = os.getenv("BIMBA3D_LEGACY_PIPELINE_ROOT")
            if env_root:
                legacy_roots.insert(0, Path(env_root))

            pipeline_dirs: list[Path] = []
            seen_dirs: set[Path] = set()
            for pipeline_dir in explicit_pipeline_dirs:
                if pipeline_dir.exists() and (pipeline_dir / "pipeline.json").exists():
                    seen_dirs.add(pipeline_dir)
                    pipeline_dirs.append(pipeline_dir)
            for root in legacy_roots:
                if not root.exists():
                    continue
                if (root / "pipeline.json").exists() and root not in seen_dirs:
                    seen_dirs.add(root)
                    pipeline_dirs.append(root)
                for child in root.iterdir():
                    if child.is_dir() and (child / "pipeline.json").exists() and child not in seen_dirs:
                        seen_dirs.add(child)
                        pipeline_dirs.append(child)

            for pipeline_path in pipeline_dirs:
                for sub_project_dir in sorted(pipeline_path.iterdir()):
                    if not sub_project_dir.is_dir() or sub_project_dir.name in ["shared_models", "training_pipelines"]:
                        continue
                    if (sub_project_dir / "outputs" / "sparse" / "0").exists():
                        scan_project_dir(sub_project_dir, projects_list)
        ...
        if include_legacy_colmap_sources:
            try:
                scan_legacy_colmap_source_pipelines(projects)
            except Exception as e:
                logger.warning(f"Failed to scan legacy COLMAP source projects: {e}")

        deduped_projects: list[ProjectListItem] = []
...
def list_reusable_models():
    ...
    try:
        ...
        allowed_modes = {"exif_compact_featurewise"}
        for item in items:
            profile = model_registry.resolve_model_ai_profile(item)
            if str(profile.get("ai_input_mode") or "").strip().lower() not in allowed_modes:
                continue
            enriched.append({
                **item,
                "ai_profile": profile,
...
def process_project(project_id: str, params: ProcessParams | None = Body(None)):
    ...
    try:
        ...
        params_payload.setdefault("tune_end_step", params_payload.get("max_steps", 15000))
        ...
        requested_ai_input_mode = _normalize_ai_input_mode_token(requested_params.get("ai_input_mode"))
        ...
        if requested_ai_input_mode and requested_ai_input_mode not in valid_ai_input_modes:
            raise HTTPException(
                status_code=400,
                detail=f"ai_input_mode must be one of: {', '.join(sorted(valid_ai_input_modes))}",
            )
        ...
        if engine == "gsplat" and mode_value == "modified" and tune_scope_value == "core_ai_optimization":
            chosen_mode = requested_ai_input_mode or _normalize_ai_input_mode_token(params_payload.get("ai_input_mode"))
            if chosen_mode:
        ...
        _workflow_source_model_for_mode_check = None
        if _requested_source_model_id:
            ...
            if not isinstance(_source_model_record_for_mode_check, dict):
                _workflow_source_model_for_mode_check = workflow_model_seeding.read_workflow_model(_requested_source_model_id)
        _source_is_learner_only = (
            isinstance(_source_model_record_for_mode_check, dict)
            and str(_source_model_record_for_mode_check.get("artifact_format") or "").strip().lower() in {"learner_json", "neural_pt"}
            and not (_source_model_record_for_mode_check.get("paths") or {}).get("checkpoint")
        )
        _source_is_workflow_quality_model = _workflow_source_model_for_mode_check is not None

        if is_session_test and (_source_is_learner_only or _source_is_workflow_quality_model):
            # Quality models drive parameter selection only; gsplat itself starts from scratch.
        ...
        # the Featurewise Ridge/MLP selectors can find them during the test run.
        if is_session_test and _source_is_workflow_quality_model and _workflow_source_model_for_mode_check is not None:
            _workflow_profile = workflow_model_seeding.model_ai_profile(_workflow_source_model_for_mode_check)
            inherited_test_ai_input_mode = _normalize_ai_input_mode_token(_workflow_profile["ai_input_mode"])
            inherited_test_ai_selector_strategy = _workflow_profile["ai_selector_strategy"]
            params_payload["source_model_id"] = _workflow_source_model_for_mode_check.model_id
            params_payload["source_workflow_model_id"] = _workflow_source_model_for_mode_check.model_id
            params_payload["source_model_name"] = _workflow_source_model_for_mode_check.model_name
            params_payload["source_model_family"] = _workflow_source_model_for_mode_check.model_family
            params_payload["source_training_data_id"] = _workflow_source_model_for_mode_check.source_training_data_id
            _model_evaluation_step = workflow_model_seeding.model_evaluation_step(_workflow_source_model_for_mode_check)
            if not _model_evaluation_step:
                raise HTTPException(
                    status_code=400,
                    detail=(
            ...
            params_payload["model_evaluation_step"] = _model_evaluation_step
            params_payload["score_reference_step"] = _model_evaluation_step
            try:
                _seeded_path = workflow_model_seeding.seed_workflow_model_into_project(
                    _workflow_source_model_for_mode_check,
                    project_dir,
                )
                params_payload["source_model_artifact_path"] = str(_seeded_path)
                logger.info(
                    "Seeded workflow model '%s' into project %s at %s",
                    _workflow_source_model_for_mode_check.model_id,
                    project_id,
                    _seeded_path,
                )
            except Exception as _seed_exc:
                raise HTTPException(status_code=400, detail=str(_seed_exc))
        ...
        if is_session_test and _source_is_learner_only and isinstance(_source_model_record_for_mode_check, dict):
            ...
            inherited_test_ai_input_mode = _normalize_ai_input_mode_token(_learner_profile.get("ai_input_mode")) or None
            inherited_test_ai_selector_strategy = (
                str(_learner_profile.get("ai_selector_strategy") or "").strip().lower() or None
            )
            # Also inherit the report selector strategy explicitly from ai_profile.
            ...
            # Seed learner weights into the project's local report model directory.
            try:
        ...
        if start_model_mode == "reuse":
            ...
            if source_model_id:
                ...
                if is_session_test:
                    ...
                    inherited_test_ai_input_mode = _normalize_ai_input_mode_token(inherited_test_profile.get("ai_input_mode")) or None
                    inherited_test_ai_selector_strategy = (
                ...
                if not raw_checkpoint or not str(raw_checkpoint).strip():
                    ...
                    if artifact_fmt in ("learner_json", "neural_pt"):
                        # Learner-only models (ridge or neural) don't have a gsplat checkpoint.
                        # They only carry learner weights that get seeded into the project.
                        # Skip checkpoint validation; the seeding already happened above.
                        # Use scratch mode (no checkpoint warm-start) since there's no .pt checkpoint.
                ...
                if is_session_test:
                    ...
                    if isinstance(source_cfg, dict):
                        ...
                        if source_mode == "modified" and source_scope == "core_ai_optimization":
                            inherited_test_ai_input_mode = _normalize_ai_input_mode_token(
                                source_resolved.get("ai_input_mode") or source_requested.get("ai_input_mode") or ""
                            ) or inherited_test_ai_input_mode
            ...
            params_payload["start_model_mode"] = "scratch"
            if not (is_session_test and _source_is_workflow_quality_model):
                params_payload.pop("source_model_id", None)
            params_payload.pop("source_run_id", None)
            params_payload.pop("source_model_checkpoint", None)
            ...
            if not project_model_name:
                if is_session_test and _source_is_workflow_quality_model and _workflow_source_model_for_mode_check is not None:
                    project_model_name = _workflow_source_model_for_mode_check.model_name
                else:
                    fallback_name = str(requested_params.get("run_name") or "").strip()
                    project_model_name = fallback_name or "project_model"
            if not (is_session_test and _source_is_workflow_quality_model):
                # Scratch training starts a new project-scoped model series.
        ...
        def run_project_pipeline_with_training_data_update() -> None:
            try:
                pipeline.run_full_pipeline(project_id, params_payload)
                if not bool(params_payload.get("update_training_data")):
                    return

                target_id = str(params_payload.get("training_data_target_id") or "").strip()
                if not target_id:
                    raise ValueError("update_training_data is enabled but no Training Data target was selected.")

                final_status = status.get_status(project_id)
                if str(final_status.get("status") or "").lower() != "completed":
                    logger.info(
                        "Skipping Training Data update for project %s run %s because final status is %s",
                        project_id,
                        run_id,
                ...
                updated_manifest = project_training_data_update.append_project_run_to_training_data(
                    project_dir=project_dir,
                    project_id=project_id,
                    run_id=run_id,
                    training_data_id=target_id,
                    params=params_payload,
                )
                _write_json_atomic(
                    project_dir / TRAINING_DATA_UPDATE_MARKER_FILE,
                    {
                        "updated_at": datetime.utcnow().isoformat() + "Z",
                        "project_id": project_id,
                        "run_id": run_id,
                        "training_data_id": updated_manifest.training_data_id,
                        "training_data_name": updated_manifest.name,
                        "row_count": updated_manifest.row_count,
                        "source_workflow_model_id": params_payload.get("source_workflow_model_id") or params_payload.get("source_model_id"),
                        "source_model_name": params_payload.get("source_model_name"),
                        "source_model_family": params_payload.get("source_model_family") or params_payload.get("ai_selector_strategy"),
                        "model_evaluation_step": params_payload.get("model_evaluation_step"),
                        "score_reference_step": params_payload.get("score_reference_step"),
                    },
                )
                status.update_status(
                    project_id,
                    ...
                    progress=100,
                    message=(
                        f"Training Data updated: {updated_manifest.name} "
                        f"({updated_manifest.row_count} rows)."
                    ),
                )
            except Exception as exc:
                logger.error(
                    "Project %s run %s failed during Training Data update: %s",
                    project_id,
                    run_id,
                    exc,
                )
                status.update_status(project_id, "failed", error=str(exc), message=str(exc))

        # Start processing in background thread.
        thread = threading.Thread(
            target=run_project_pipeline_with_training_data_update,
            daemon=True
        )
        thread.start()
...
def get_project_telemetry(
    ...
    try:
        ...
        # Load project-level EXIF features (ONLY source of truth)
        project_exif_file = project_dir / "exif_features.json"
        project_exif_features = None
        if project_exif_file.exists():
            try:
                with open(project_exif_file, "r", encoding="utf-8") as fh:
                    project_exif_data = json.load(fh)
                if isinstance(project_exif_data, dict):
                    project_exif_features = project_exif_data.get("features")
                    logger.debug(f"Loaded EXIF from project-level file for {project_id}")
            except Exception as exc:
                logger.warning(f"Failed to load project-level EXIF for {project_id}: {exc}")
        else:
            logger.debug(f"No project-level EXIF file for {project_id} - will show empty features")
        ...
        if isinstance(run_analytics, dict):
            ...
            if isinstance(canonical_ai, dict) and canonical_ai.get("ai_input_mode"):
                ...
                # CRITICAL: ONLY use project-level EXIF features (single source of truth)
                # No fallback - if project-level file doesn't exist, features will be empty
                raw_features = project_exif_features if isinstance(project_exif_features, dict) else {}

                raw_feature_sources = _infer_feature_sources(raw_features)
                normalized_ai["feature_details"] = raw_features
...
def get_project_ai_learning_table(project_id: str):
    """Get AI learning table for a single project.

    This endpoint returns learning data for all runs within ONE project.
    It reads score and multiplier details from canonical run analytics.

    Returns:
        - Run-level metrics (PSNR, SSIM, LPIPS, Loss)
        - Score breakdown (l, q, t, s for run best/end and base best/end)
        - Time values (elapsed time for each step)
        - Learning parameters (actual, selected, log-space, final multipliers)
    """
    try:
        ...
        logger.error("Error building project AI learning table for %s: %s", project_id, exc)
        raise HTTPException(status_code=500, detail="Failed to build project AI learning table")
...
def list_project_runs(project_id: str):
    ...
    try:
        ...
        for run_dir in sorted((p for p in runs_root.iterdir() if p.is_dir()), key=lambda p: p.name, reverse=True):
            ...
            is_completed = has_completed_outputs or has_analytics_summary or (is_base_run and project_has_completed_outputs)
            ...
            try:
                ...
                logger.warning("Skipping AI log scan for run %s due to access error: %s", run_id, exc)
        ...
        if runs and (not base_session_id or not any(r["run_id"] == base_session_id for r in runs)):
            fallback_base = next(
                (
                    ...
                    and "_phase1_run1_" in item["run_id"]
                    and item.get("session_status") == "completed"
                ),
                runs[0]["run_id"],
            )
            try:
...
def get_experiment_summary(
    ...
    try:
        ...
        run_analytics = _read_run_analytics(resolved_run_dir) if resolved_run_dir else None
        ...
        return {
            ...
            "ai_insights": _analytics_ai_insights(run_analytics),
        }
