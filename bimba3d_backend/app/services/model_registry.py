...
from bimba3d_backend.worker.ai_input_modes.legacy_support import normalize_ai_input_mode_token
...
def find_best_checkpoint_in_pipeline_folder(pipeline_folder: Path) -> Path | None:
    ...
    best (highest-score or most-recent) gsplat checkpoint .pt file found.

    Search order:
    1. Runs with the highest score in canonical run analytics.
    2. Falls back to the most recently modified checkpoint file across all runs.
    """
    best_checkpoint: Path | None = None
    best_relative_score: float | None = None
        ...
        for run_dir in runs_dir.iterdir():
            ...
            if analytics_file.exists():
                try:
                    ...
                    results = ai_block.get("input_mode_learning") if isinstance(ai_block, dict) else {}
            ...
            if best_checkpoint is None:
                ...
                best_mtime = mtime
            elif score is not None and (best_relative_score is None or score > best_relative_score):
                best_checkpoint = ckpt
                best_relative_score = score
                best_mtime = mtime
            elif score is None and best_relative_score is None and mtime > best_mtime:
                best_checkpoint = ckpt
                best_mtime = mtime
...
def seed_learner_weights_into_project(
    ...
    """Copy registered learner weights into the target project's local model directory.

    The selector loads Ridge weights from:
      project_dir/models/featurewise_ridge_regression/{mode}.json
    For MLP models:
      project_dir/models/featurewise_mlp/{mode}.pt

    This function is called by the API before dispatching a test run when the
    selected source model has artifact_format='learner_json' or 'neural_pt'.
    ...
    # Locate the learner file â€” prefer paths.artifact, then scan model_dir
    paths = model_record.get("paths") if isinstance(model_record.get("paths"), dict) else {}
    ...
    if learner_src is None:
        if is_neural:
            # Scan for .pt file
            candidate = model_dir / f"learner_{mode}.pt"
            if candidate.is_file():
                learner_src = candidate
        else:
    ...
    if learner_src is None:
        raise FileNotFoundError(
            f"Cannot seed learner model: source artifact not found in model record {model_record.get('model_id')}"
        )
    ...
    try:
        ...
        raise IOError(f"Failed to seed learner model to {dest_path}: {e}")
...
def resolve_model_ai_profile(model_record: dict | None) -> dict[str, str]:
    ...
    explicit_mode = normalize_ai_input_mode_token(explicit_profile.get("ai_input_mode"))
    ...
    def _infer_selector_strategy_from_metadata(cfg_path: Path) -> str | None:
        ...
        if selected_preset in valid_selector_strategies:
        ...
        if yhat_keys & continuous_keys:
            return "featurewise_ridge_regression"
        if yhat_keys & preset_keys or selected_preset in preset_keys:
            return None
        return None
    ...
    for cfg_path in candidate_paths:
        ...
        ai_input_mode = normalize_ai_input_mode_token(resolved.get("ai_input_mode") or requested.get("ai_input_mode") or "")
