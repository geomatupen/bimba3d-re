...
# Configure temp directory to use E drive to avoid filling C drive
E_TEMP_DIR = Path("E:/Thesis/Temp")
...
os.environ.setdefault('XDG_CACHE_HOME', str(E_TEMP_DIR / '.cache'))
os.environ.setdefault('TORCH_HOME', str(E_TEMP_DIR / '.cache' / 'torch'))
os.environ.setdefault('MPLCONFIGDIR', str(E_TEMP_DIR / '.cache' / 'matplotlib'))
os.environ.setdefault('HF_HOME', str(E_TEMP_DIR / '.cache' / 'huggingface'))
...
def run_colmap(image_dir: Path, output_dir: Path, params: dict | None = None) -> Path:
    ...
    if stop_flag.exists():
        update_status(output_dir.parent, "stopped", progress=0, stop_requested=True, stage="colmap", message="Processing stopped by user before feature extraction.", stopped_stage="colmap")
        try:
            stop_flag.unlink()
        except Exception:
            pass
    ...
    if stop_flag.exists():
        update_status(output_dir.parent, "stopped", progress=0, stop_requested=True, stage="colmap", message="Processing stopped by user before feature matching.", stopped_stage="colmap")
        try:
            stop_flag.unlink()
        except Exception:
            pass
    ...
    if stop_flag.exists():
        update_status(output_dir.parent, "stopped", progress=0, stop_requested=True, stage="colmap", message="Processing stopped by user before sparse reconstruction.", stopped_stage="colmap")
        sys.exit(0)
                ...
                if stop_flag.exists():
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass
                    update_status(output_dir.parent, "stopped", progress=0, stop_requested=True, stage="colmap", message="Processing stopped by user during sparse reconstruction.", stopped_stage="colmap")
                    try:
...
def _collect_eval_history(engine_output_dir: Path, params: dict, mode: str, elapsed_by_step: dict | None = None, loss_by_step: dict | None = None) -> list[dict]:
    """Build comparison-compatible eval history from upstream stats outputs."""
    stats_dir = engine_output_dir / "stats"
    if not stats_dir.exists():
        return []

    elapsed_by_step = elapsed_by_step or {}
    ...
    for stats_file in sorted(stats_dir.glob("val_step*.json")):
        ...
        # Try to get elapsed_seconds from elapsed_by_step
        if step in elapsed_by_step:
            elapsed_seconds = float(elapsed_by_step[step])

        # Try to get final_loss from loss_by_step
        if step in loss_by_step:
            final_loss = float(loss_by_step[step])
...
def _run_selected_training_engine(
    ...
    *,
    ...
    try:
        ...
        root_error_lower = root_error.lower()
        is_cuda_toolchain_issue = (
            "cuda" in root_error_lower
            or "cudart" in root_error_lower
            or "cublas" in root_error_lower
            or "cudnn" in root_error_lower
            or "nvrtc" in root_error_lower
            or "nvcuda" in root_error_lower
            or "winerror 193" in root_error_lower
            or "not a valid win32 application" in root_error_lower
            or "torch.cuda" in root_error_lower
        )

        if os.name == "nt" and is_cuda_toolchain_issue:
            if "winerror 193" in root_error_lower or "not a valid win32 application" in root_error_lower:
                guidance = (
        ...
        elif os.name != "nt" and is_cuda_toolchain_issue:
            guidance = (
            ...
            guidance = "gsplat training failed before CUDA execution. Check the root error and seeded model payload."
...
def main():
    logger.info("Worker temp directory: %s", tempfile.gettempdir())
    ...
    try:
        ...
        terminal_state = str(stop_reason.get("terminal_state") or "").strip().lower() if isinstance(stop_reason, dict) else ""
        terminal_failure = terminal_state == "failed"
        terminal_partial = terminal_state == "partial_completed"
        if terminal_failure:
            ...
            partial_message = str(stop_reason.get("message") or "Training partially completed.")
            update_status(
                project_dir,
                "partial_completed",
                stage="training",
                ...
                gaussian_cap_reached=stop_reason.get("reason") == "gaussian_hard_cap_reached",
                gaussian_cap_step=stop_reason.get("step"),
                relative_quality_score=stop_reason.get("score"),
                quality_score_source="gaussian_cap_penalty",
                exclude_from_normalization=True,
                valid_completed_quality=False,
            )
            logger.warning("Pipeline partially completed with engine payload: %s", stop_reason)
            stop_reason = None
        elif stop_reason:
            ...
            if stop_source == "backend":
                final_message = f" Processing stopped by backend shutdown at step {stop_reason}."
            else:
                final_message = f" Processing stopped by user at step {stop_reason}."
            ...
            if stop_requested_flag:
                ...
                if stop_source == "backend":
                    final_message = " Processing stopped by backend shutdown."
                else:
