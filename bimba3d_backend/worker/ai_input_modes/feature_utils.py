...
def get_sensor_dimensions() -> dict[str, float]:
    ...
    NOTE: Sensor sizes verified from actual image EXIF data in training pipeline.
    """
    return {
        # DJI Mavic 3 series (Hasselblad cameras)
        "M3E": (17.3, 13.0),          # Mavic 3 Enterprise - 4/3" sensor, focal 12.29mm
        "M3T": (9.6, 7.2),            # Mavic 3 Thermal - 1/2" sensor, focal 4.4mm (wide camera)
        "M4E": (17.3, 13.0),          # Mavic 4 Enterprise - 4/3" sensor, focal 12.29mm
        "DJI_FC6310": (17.3, 13.0),   # Mavic 3 main camera (older naming)
        "DJI_FC6D": (17.3, 13.0),     # Mavic 3 Classic

        # DJI Mavic 2 series
        "DJI_FC3170": (13.2, 8.8),    # Mavic 2 Pro - 1" sensor

        # DJI Mini/Air series (smaller sensors)
        "FC2403": (9.6, 7.2),         # Mavic 2 Enterprise / Mini series - 1/2" sensor, focal 4.5mm
        "FC7503": (9.6, 7.2),         # Air series - 1/2" sensor, focal 4.49mm
        "DJI_FC6510": (9.6, 7.2),     # Air 3
        ...
        # Other manufacturers
        "EP800": (13.2, 8.8),         # Enterprise drone - 1" sensor, focal 8.8mm
...
def infer_sensor_width(camera_model: str) -> float:
    ...
    Returns:
        Estimated sensor width in mm, or 13.2 (DJI 1" sensor default) if unknown.
    """
    sensors = get_sensor_dimensions()

    # Exact match on model identifier
    for model, (w, _) in sensors.items():
        if model in camera_model:
            return w

    # Substring matching for common brands/series
    model_lower = camera_model.lower()

    # DJI specific model series detection
    if "m3e" in model_lower or "m4e" in model_lower or "mavic 3 enterprise" in model_lower:
        return 17.3  # 4/3" sensor (Hasselblad camera)
    elif "m3t" in model_lower or "mavic 3 thermal" in model_lower:
        return 9.6   # 1/2" sensor (wide camera)
    elif "fc2403" in model_lower or "fc7503" in model_lower:
        return 9.6   # 1/2" sensor (Mini/Air series)
    elif "mavic 3" in model_lower:
        return 17.3  # Default to 4/3" for Mavic 3
    elif "mavic 2" in model_lower or "fc3170" in model_lower:
        return 13.2  # 1" sensor (Mavic 2 Pro)
    elif "mavic" in model_lower or "dji" in model_lower:
        return 13.2  # Generic DJI default (1" sensor)
    elif "phantom" in model_lower:
        return 13.2  # 1" sensor
    elif "mini" in model_lower or "air" in model_lower:
        return 9.6   # 1/2" sensor

    # Other manufacturers
    elif "canon" in model_lower or "nikon" in model_lower:
        return 23.5  # DSLR/mirrorless: assume APS-C
    elif "iphone" in model_lower or "samsung" in model_lower:
        return 8.0   # Smartphone sensor

    # Default to DJI 1" sensor (most common for UAV photogrammetry)
    return 13.2
