"""Runtime and training helpers for compact_descriptor_mlp.

compact_descriptor_mlp is the simple 13-input MLP comparison model:
10 standardized compact scene descriptors + 3 log multiplier candidates.
It intentionally does not use the engineered x*m interaction terms or m^2
terms used by compact_featurewise_mlp.
"""
...
DEFAULT_EPOCHS = 1000
DEFAULT_PATIENCE = 50
# Stored as a fallback only; explicit testing candidate grids override this value.
DEFAULT_CANDIDATE_POINTS = 30
DEFAULT_SEED = 42


if HAS_TORCH:
    class CompactDescriptorMLP(nn.Module):
        """One score model over descriptors and three joint multiplier actions."""
    ...
    class CompactDescriptorMLP:  # pragma: no cover
        def __init__(self, *args: Any, **kwargs: Any):
            raise RuntimeError("PyTorch is required for compact_descriptor_mlp.")


def train_compact_descriptor_mlp_model(
    *,
    ...
    # The seed fixes the project split, initial weights, dropout sequence, and
    # optimizer randomness so repeated experiments are comparable.
    torch.manual_seed(DEFAULT_SEED)
    np.random.seed(DEFAULT_SEED)

    bounds = normalise_compact_group_bounds(group_bounds)
    prepared = _prepare_compact_descriptor_mlp_dataset(training_data=training_data, bounds=bounds)
    if not prepared["ok"]:
    ...
    model = CompactDescriptorMLP(input_dim=X.shape[1], hidden=DEFAULT_HIDDEN, dropout=DEFAULT_DROPOUT)
    ...
    model_dir = save_dir / "compact_descriptor_mlp"
    model_dir.mkdir(parents=True, exist_ok=True)
    model_path = model_dir / f"compact_descriptor_{timestamp}.pt"
    checkpoint = {
        "state_dict": model.state_dict(),
        "model_type": "compact_descriptor_mlp",
        "input_representation": "10_standardized_descriptors_plus_3_log_multipliers",
    ...
    metadata = {
        ...
        "input_representation": checkpoint["input_representation"],
        "input_dim": int(X.shape[1]),
        "hidden": DEFAULT_HIDDEN,
        "dropout": DEFAULT_DROPOUT,
        "training_samples": int(n),
    ...
    metadata_path = model_dir / f"compact_descriptor_{timestamp}_metadata.json"
...
def _prepare_compact_descriptor_mlp_dataset(
    *,
    training_data: list[dict[str, Any]],
    bounds: dict[str, tuple[float, float]],
) -> dict[str, Any]:
    """Build tensors using only compact descriptors and log multipliers."""
    ...
    for entry in training_data:
        ...
        if action_logs is None:
            continue
        valid_entries.append({"features": dict(features), "action_logs": action_logs, "project_name": project_name.strip()})
        scores.append(float(score))
        project_names.append(project_name.strip())

    if not scores:
        return {"ok": False, "error": "No valid compact descriptor MLP score-training rows available"}
    if len(set(project_names)) < 2:
        return {"ok": False, "error": "compact_descriptor_mlp project-level validation needs at least 2 projects"}
    ...
    design_rows = [
        _build_descriptor_design_vector(
...
def _build_descriptor_design_vector(x: np.ndarray, action_logs: np.ndarray) -> np.ndarray:
    """Return the 13-value simple MLP input: descriptors followed by actions."""
    # build_compact_vector includes a leading intercept for Ridge/expanded MLP.
    # This simple comparison model must use only the 10 descriptor values, so
    # remove that intercept before appending the 3 log multiplier actions.
    descriptors = x.astype(np.float64)
    if descriptors.shape[0] == 11:
        descriptors = descriptors[1:]
    return np.concatenate([descriptors, action_logs.astype(np.float64)], axis=0)
...
def predict_compact_descriptor_mlp_multipliers(
    *,
    ...
    if not HAS_TORCH:
        raise RuntimeError("PyTorch is required for compact_descriptor_mlp prediction.")
    model_path = _latest_model_path(shared_models_dir)
    if not model_path.exists():
        raise FileNotFoundError(f"compact_descriptor_mlp model not found: {model_path}")
    checkpoint = torch.load(model_path, map_location="cpu", weights_only=False)
    return predict_compact_descriptor_mlp_from_checkpoint(
...
def predict_compact_descriptor_mlp_from_checkpoint(
    *,
    ...
    batch = np.stack([_build_descriptor_design_vector(x, np.array(combo, dtype=np.float64)).astype(np.float32) for combo in combos], axis=0)
    with torch.no_grad():
...
def _latest_model_path(shared_models_dir: Path) -> Path:
    model_dir = shared_models_dir / "compact_descriptor_mlp"
    versioned = sorted(model_dir.glob("compact_descriptor_*.pt"), key=lambda p: p.stem, reverse=True)
    if versioned:
        return versioned[0]
    return model_dir / "compact_descriptor.pt"
