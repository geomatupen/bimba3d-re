...
import { Play, Settings2, Layers, Map as MapIcon, Boxes, Check, X, Clock, Square, Download } from "lucide-react";
...
import type { SessionExecutionMode } from "./CoreAiSessionTestControls";
import { InfoIcon as Info } from "../projectProcess/InfoIcon";
import { ProcessToasts } from "../projectProcess/ProcessToasts";
import type {
 AiInputMode,
 AiSelectorStrategy,
 EngineOutputBundle,
 NewSessionConfigSource,
 PreviewFile,
 ProjectRunInfo,
 ReusableModelEntry,
 RunJitterMode,
 ...
 StartModelMode,
 TelemetryPayload,
 TrainingDataTargetOption,
 TrainingEngine,
 TrendScope,
 TuneScope,
} from "../projectProcess/processTypes";
import {
 buildDefaultRunName,
 ...
 formatEngineLabel,
 formatTelemetryFieldLabel,
 formatTelemetryScalar,
 getDefaultProcessConfig,
 LEARNABLE_AI_PARAM_KEYS,
 sanitizeFilenameToken,
 sanitizeRunToken,
} from "../projectProcess/processUtils";
...
const AI_SELECTOR_LABELS: Record<AiSelectorStrategy, string> = {
 featurewise_ridge_regression: "Featurewise Ridge Regression",
...
export default function ProcessTab({ projectId }: ProcessTabProps) {
 ...
 const [tuneScope, setTuneScope] = useState<TuneScope>(cfg.tune_scope ?? "core_ai_optimization");
 ...
 cfg.ai_input_mode === "exif_compact_featurewise"
 ? cfg.ai_input_mode
 ...
 const [updateTrainingData, setUpdateTrainingData] = useState<boolean>(Boolean(cfg.update_training_data));
 const [trainingDataTargetId, setTrainingDataTargetId] = useState<string>(cfg.training_data_target_id || "");
 const [trainingDataTargets, setTrainingDataTargets] = useState<TrainingDataTargetOption[]>([]);
 const [trainingDataTargetsLoading, setTrainingDataTargetsLoading] = useState<boolean>(false);
 ...
 const selectedTrainingDataTargetValid =
 Boolean(trainingDataTargetId) && trainingDataTargets.some((target) => target.id === trainingDataTargetId);
 const shouldUpdateTrainingData = updateTrainingData && selectedTrainingDataTargetValid;
 ...
 mode: 'Training profile. Baseline Run keeps default gsplat behavior; AI Guided Run tests a selected workflow model through its AI profile.',
 tune_start_step: 'For AI Guided Run mode, this is the first step where tuning checks are allowed. Before this step, no rule-based LR updates are applied.',
 ...
 tune_end_step: 'For AI Guided Run mode, this is the last step where rule-based tuning updates are allowed. The worker keeps applying rule checks until this step, then continues normal training.',
 tune_interval: 'For AI Guided Run mode, worker evaluates and applies rule-based updates every N steps during the tuning window.',
 ...
 densification_interval: 'Spacing between densification steps. This value is configurable from frontend in both modes. [original]',
 ...
 // --- CUSTOM PARAMETERS ---
 engine: 'gsplat renderer used by the project processing flow. [custom]',
 ...
 update_training_data: 'After the project test run, mark a valid final Training Data artifact for update using this project result. [custom]',
 training_data_target_id: 'Final Training Data artifact from the workflow Training Data page that should receive this project test result when update training data is enabled. [custom]'
 };
 ...
 if (sessionExecutionMode !== "test") {
 setSessionExecutionMode("test");
 }
 if (tuneScope !== "core_ai_optimization") {
 setTuneScope("core_ai_optimization");
 }
 if (aiInputMode !== "exif_compact_featurewise") {
 setAiInputMode("exif_compact_featurewise");
 }
 ...
 setTuneScope(defaults.tune_scope ?? "core_ai_optimization");
 ...
 setUpdateTrainingData(defaults.update_training_data);
 setTrainingDataTargetId(defaults.training_data_target_id);
 ...
 if (resolved.ai_input_mode === "exif_compact_featurewise") {
 setAiInputMode(resolved.ai_input_mode as AiInputMode);
 }
 if (isAiSelectorStrategy(resolved.ai_selector_strategy)) {
 ...
 if (typeof resolved.update_training_data === "boolean") setUpdateTrainingData(resolved.update_training_data);
 if (typeof resolved.training_data_target_id === "string") setTrainingDataTargetId(resolved.training_data_target_id);
 ...
 const loadTrainingDataTargets = async () => {
 setTrainingDataTargetsLoading(true);
 try {
 const res = await api.get("/api/workflow/training-data", { params: { usable_only: true } });
 const items = Array.isArray(res.data?.items) ? res.data.items : [];
 const nextTargets: TrainingDataTargetOption[] = items
 .map((item: Record<string, unknown>) => {
 const id = String(item.training_data_id || item.id || "");
 const name = String(item.name || id);
 const rowCount = Number(item.row_count || 0);
 return {
 id,
 label: name || id,
 detail: rowCount > 0 ? `${rowCount} row${rowCount === 1 ? "" : "s"}` : undefined,
 };
 })
 .filter((item: TrainingDataTargetOption) => item.id);

 if (!cancelled) {
 setTrainingDataTargets(nextTargets);
 if (!nextTargets.some((target) => target.id === trainingDataTargetId)) {
 setTrainingDataTargetId(nextTargets[0]?.id || "");
 }
 if (nextTargets.length === 0) {
 setUpdateTrainingData(false);
 }
 }
 } catch (err) {
 console.error("Failed to load training data targets", err);
 if (!cancelled) {
 setTrainingDataTargets([]);
 setTrainingDataTargetId("");
 setUpdateTrainingData(false);
 }
 } finally {
 if (!cancelled) {
 setTrainingDataTargetsLoading(false);
 }
 }
 };

 loadTrainingDataTargets();
 return () => {
 cancelled = true;
 };
 }, [projectId]);
 ...
 const rootPreviewList = parsePreviewItems(files.previews);
 const rootSnapshotSource = Array.isArray(files.model_snapshots)
 ? files.model_snapshots
 : Array.isArray(files.ply)
 ? files.ply
 : [];
 const rootSnapshots = buildSnapshotEntries(rootSnapshotSource);
 ...
 const previewList = activeEngineBundle ? activeEngineBundle.previews : rootPreviewList;
 setPngFiles(previewList);
 setSelectedPng((prev) => (prev && previewList.some((item) => item.url === prev) ? prev : null));

 const snapshotEntries = activeEngineBundle ? activeEngineBundle.snapshots : rootSnapshots;
 ...
 if (showCoreAiSessionControls && !sourceModelId) {
 setError("Select a model for the AI Guided Run.");
 setProcessing(false);
 return;
 }
 ...
 ["score_positive", aiInsights.score_positive],
 ["score_label", aiInsights.score_label],
 ["score_mode", aiInsights.score_mode],
 ["score_preset", aiInsights.score_preset],
 ]);
 addConfigTable(
 ...
 <ProcessToasts configSavedToast={configSavedToast} processInfoToast={processInfoToast} />
 {!gpuAvailable && (
 ...
 <div>Score value: <span className="font-semibold">{formatTelemetryScalar(telemetryData.ai_insights.score)}</span></div>
 <div>Positive score: <span className="font-semibold">{formatTelemetryScalar(telemetryData.ai_insights.score_positive)}</span></div>
 <div>Score label: <span className="font-semibold">{formatTelemetryScalar(telemetryData.ai_insights.score_label)}</span></div>
 </div>
 ...
 <div>
 <p className="text-xs font-semibold text-slate-700 mb-2">Extracted descriptor features</p>
 ...
 <select
 value={engine}
 className="w-full px-2 py-1.5 text-xs border border-slate-300 rounded-md bg-slate-100 text-slate-700"
 disabled
 >
 <option value="gsplat">gsplat (default pipeline)</option>
 </select>
 </div>
 <div className="md:col-span-2 rounded-md border border-slate-200 bg-white p-2">
 <label className="flex items-center justify-between gap-2 text-[11px] font-medium text-slate-700">
 <span className="inline-flex items-center gap-2">
 <input
 type="checkbox"
 checked={updateTrainingData}
 onChange={(e) => {
 const enabled = e.target.checked;
 setUpdateTrainingData(enabled);
 if (enabled && !trainingDataTargetId && trainingDataTargets[0]) {
 setTrainingDataTargetId(trainingDataTargets[0].id);
 }
 }}
 disabled={trainingDataTargetsLoading || trainingDataTargets.length === 0}
 className="w-4 h-4"
 />
 Update training data
 </span>
 <button type="button" onClick={() => setSelectedInfoKey("update_training_data")} className="p-1 text-slate-400 hover:text-slate-600"><Info /></button>
 </label>
 {updateTrainingData && (
 <div className="mt-2">
 <label className="flex items-center justify-between text-[11px] font-medium text-slate-600 mb-0.5">
 <span>Training data target</span>
 <button type="button" onClick={() => setSelectedInfoKey("training_data_target_id")} className="p-1 text-slate-400 hover:text-slate-600"><Info /></button>
 </label>
 <select
 value={trainingDataTargetId}
 onChange={(e) => setTrainingDataTargetId(e.target.value)}
 ...
 <option value="">No valid Training Data available</option>
 )}
 {trainingDataTargets.map((target) => (
 ...
 {trainingDataTargets.length === 0
 ? "Build final Training Data in Research Workflow before enabling project-level updates."
 : "Only ready final Training Data artifacts from the workflow are listed here."}
 </p>
 </div>
 )}
 </div>
 ...
 disabled={processing || isStopping}
 >
 <option value="baseline">Baseline Run</option>
 <option value="modified">AI Guided Run</option>
 </select>
 </div>
 )}
 ...
 setSourceModelId(nextModelId);
 if (nextModelId) {
 setStartModelMode("reuse");
 }
 }}
 disabled={reusableModelsLoading || modeCompatibleReusableModels.length === 0}
 ...
 <p className="mt-1 text-[10px] text-slate-500">
 Uses the selected model's AI profile; choose the Ridge or MLP model to guide this test run.
 </p>
 ...
 <span className="text-[10px] text-blue-700">Test only</span>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
 <div className="md:col-span-2 rounded-md border border-blue-200 bg-blue-50 p-2 text-[11px] text-blue-900">
 Project AI Guided Run only tests a selected workflow model. Use Update training data to collect new rows, then train featurewise or compact models later from Workflow.
 </div>
 {/* Baseline and selector controls */}
 ...
 className="w-full px-2 py-1.5 text-xs border border-slate-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
 >
 {AI_SELECTOR_STRATEGIES.map((strategy) => (
 <option key={strategy} value={strategy}>
 {AI_SELECTOR_LABELS[strategy]}
 </option>
 ))}
 </select>
 <p className="mt-1 text-[10px] text-slate-500">
 {aiSelectorStrategy === "featurewise_mlp"
 ...
 : "Optional in test mode. Leave empty to run without baseline-relative score/base-S comparison."}
 </p>
 </div>
 )}
 {sessionExecutionMode === "train" && (
 ...
 <div>
 <label className="flex items-center justify-between text-[11px] font-medium text-slate-600 mb-0.5">
 <span>AI guided tuning start step</span>
 ...
 <div>
 <label className="flex items-center justify-between text-[11px] font-medium text-slate-600 mb-0.5">
 <span>AI guided tuning end step</span>
 ...
 <div>
 <label className="flex items-center justify-between text-[11px] font-medium text-slate-600 mb-0.5">
 <span>AI guided tuning interval</span>
