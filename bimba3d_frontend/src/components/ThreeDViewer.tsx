...
export default function ThreeDViewer({
  ...
  function initSplatRenderer(target: SceneTarget) {
    ...
    try {
      ...
      // Set up TrackballControls for unconstrained 360° rotation.
      // TrackballControls uses quaternion-based rotation with no "up" axis lock,
      // unlike OrbitControls which clamps vertical rotation (phi) between 0 and π.
      let trackball: TrackballControls | null = null;
      const setupTrackball = () => {
        const camera = viewer.camera;
        const domElement = viewer.renderer?.domElement as HTMLElement | undefined;
        if (!camera || !domElement) return;

        trackball = new TrackballControls(camera, domElement);
        ...
        // Monkey-patch viewer.update to call trackball.update() each frame,
        // keeping it in sync with the splat sorting/rendering loop.
      ...
      // The viewer's camera is available after start(), but we wait one frame
      // to ensure the renderer and camera are fully initialized.
      requestAnimationFrame(setupTrackball);
