...
set "PHASE_LOG=%ProgramData%\Bimba3D\Logs\runtime-bootstrap-torch.log"
...
>>"%WRAPPER_LOG%" echo [%date% %time%] START install="%INSTALL_DIR%" phase="torch"
>>"%WRAPPER_LOG%" echo [%date% %time%] INFO phase_log="%PHASE_LOG%"
>>"%PHASE_LOG%" echo.
>>"%PHASE_LOG%" echo ===== [%date% %time%] RUN START phase=torch install="%INSTALL_DIR%" =====
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%BOOTSTRAP_ROOT%\runtime-bootstrap.ps1" -InstallDir "%INSTALL_DIR%" -Phase "torch" >>"%PHASE_LOG%" 2>&1
set "RC=%ERRORLEVEL%"
>>"%WRAPPER_LOG%" echo [%date% %time%] EXIT code=%RC% phase="torch"
>>"%PHASE_LOG%" echo ===== [%date% %time%] RUN END phase=torch exit=%RC% =====
exit /b %RC%
